                              ...
                              long int* const p_space)
{
  ...
  {
    const int num_anchors
        = option->num_anchor_scales * option->num_anchor_ratios;
    real* const anchors = (real*)malloc(num_anchors * 4 * sizeof(real));
    generate_anchors(option->anchor_scales, option->anchor_ratios, anchors,
                     option->num_anchor_scales, option->num_anchor_ratios,
                     option->base_size);
    #ifdef GPU
                      ...
                      const LayerOption* const option)
{
  ...
  const int num_anchors
      = option->num_anchor_ratios * option->num_anchor_scales;
