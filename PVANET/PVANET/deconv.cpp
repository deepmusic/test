                     ...
                     const int stride_h, const int stride_w)
{
  // thread index: (c, h, w) = c*H*W + h*W + w
    ...
    int w0 = -1;
    int h = h0 + pad_h;
    int h5_start = (h >= kernel_h) * ((h - kernel_h) / stride_h + 1);
    int h5_end = MIN(h / stride_h + 1, H5);
#endif
  ...
  for (int index = 0; index < C * H * W; ++index) {
    // parse thread index -> (c, h, w)
      ...
      if (++w0 >= W) {
          ...
          h = h0 + pad_h;
          h5_start = (h >= kernel_h) * ((h - kernel_h) / stride_h + 1);
          h5_end = MIN(h / stride_h + 1, H5);
      }
      int w = w0 + pad_w;
      int w5_start = (w >= kernel_w) * ((w - kernel_w) / stride_w + 1);
      int w5_end = MIN(w / stride_w + 1,  W5);
...
{
    global_logger.start_log(FORWARD_DECONV_LAYER);
  ...
  global_logger.stop_log(FORWARD_DECONV_LAYER);
}
