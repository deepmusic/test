                ...
                const LayerOption* const option)
{
  ...
  //   top: N x D'
  {
    ...
    //   &Z,  number of columns in Z (= D')
      ...
      int work_size[num_threads] = { N / 2, N - N / 2 };
      int offset[num_threads] = { 0, work_size[0] };
#pragma omp parallel for schedule(static,1)
      for (int i = 0; i < num_threads; i++)
          cblas_sgemm(CblasRowMajor,
          CblasNoTrans, CblasTrans,
          work_size[i], top_D, bottom_D,
          1,
          bottom2d->data + bottom_D*offset[i], bottom_D,
          weight2d->data, bottom_D,
          0,
          top2d->data + top_D*offset[i], top_D);
...
{
    global_logger.start_log(FORWARD_FC_LAYER);
  ...
  global_logger.stop_log(FORWARD_FC_LAYER);
}
