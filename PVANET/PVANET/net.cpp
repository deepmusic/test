...
{
    global_logger.start_log(FORWARD_NET_INCEPTION);

  for (int i = 0; i < net->num_layers; ++i) {
      if (i == 88) {
          global_logger.stop_log(FORWARD_NET_INCEPTION);
          global_logger.start_log(FORWARD_NET_RCNN);
      }
    for (int j = 0; j < MAX_NUM_OPS_PER_LAYER; ++j) {
  ...
  global_logger.stop_log(FORWARD_NET_RCNN);
}
