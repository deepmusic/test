   ...
   for (int kh = 0; kh < kernel_h; ++kh) {
    for (int kw = 0; kw < kernel_w; ++kw) {
      ...
      for (; h < H && h5 < H5; h += stride_h, ++h5) {
        ...
        const real* bottom3d_ptr = bottom3d + (c * H + h) * W + w;
        real* bottom5d_ptr = p_bottom5d + h5 * W5 + w5;
        int bottom3d_size = W - w;
        int bottom5d_size = W5 - w5;
        int bottom_size = (bottom3d_size < bottom5d_size) ? bottom3d_size : bottom5d_size;
        if (stride_w == 1) {
            memcpy(bottom5d_ptr, bottom3d_ptr, sizeof(real)*bottom_size);
        }
        else {
            const real* const bottom5d_end = bottom5d_ptr + bottom_size;
            while (bottom5d_ptr < bottom5d_end) {
                *bottom5d_ptr++ = *bottom3d_ptr;
                bottom3d_ptr += stride_w;
            }
        }
        w5 += bottom_size;
                  ...
                  const LayerOption* const option)
{
  ...
  for (int n = 0; n < bottom3d->num_items; ++n) {
    ...
    for (int g = 0; g < num_groups; ++g) {
      ...
      int work_size[num_threads] = { top_C / 2, top_C - top_C / 2 };
      int offset[num_threads] = { 0, work_size[0] };
#pragma omp parallel for schedule(static,1)
      for (int i = 0; i < num_threads; i++)
          cblas_sgemm(CblasRowMajor,
              CblasNoTrans,  CblasNoTrans,
              work_size[i],  top_area,  kernel_size,
              1,
              p_weight_g + kernel_size*offset[i], kernel_size,
              p_temp_g,  top_area,
              0,
              p_top_g + top_area*offset[i],  top_area);
...
{
    global_logger.start_log(FORWARD_CONV_LAYER);
  ...
  #ifdef DEBUG
  {
    ...
    global_logger.stop_log(FORWARD_CONV_LAYER);
}
