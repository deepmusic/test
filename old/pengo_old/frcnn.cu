...
void setup_frcnn_7_1_1(Net* const net)
{
  const char* names[] = {
    ...
    "rpn_score", "__deleted__", "rpn_bbox",
    "rpn_roi",

    // R-CNN: 10 layers
    "rcnn_roipool", "__deleted__",
...
void construct_frcnn_7_1_1(Net* net, int gpu_id)
{
...
void pvanet_7_1_1_init(const char* const param_path, int gpu_id)
{
  if (pvanet == NULL) {
    ...
    construct_frcnn_7_1_1(pvanet, gpu_id);
  }
}
...
void pvanet_get_output(std::vector<std::pair<std::string, std::vector<float> > >& boxes)
{
  const Tensor* const out = &pvanet->layers[40]->tops[0];
  const long int output_size = flatten_size(out);

  #ifdef GPU
  cudaMemcpyAsync(pvanet->output_cpu_data, out->data,
                  output_size * sizeof(real),
                  cudaMemcpyDeviceToHost);
  #else
  memcpy(pvanet->output_cpu_data, out->data, output_size * sizeof(real));
  #endif

  const real* const p_out_item = pvanet->output_cpu_data;
  for (int i = 0; i < out->shape[0][0]; ++i) {
...
void pvanet_7_1_1_detect(const unsigned char* image_data, int width, int height, int stride, std::vector<std::pair<std::string, std::vector<float> > >& boxes)
{
  ...
  Tensor* input = &pvanet->layers[0]->tops[0];

  input->ndim = 3;
  input->num_items = 0;
  input->start[0] = 0;
  pvanet->img_info->ndim = 1;
  pvanet->img_info->num_items = 0;

  img2input(image_data, input, pvanet->img_info,
            (unsigned char*)pvanet->temp_data,
            height, width, stride);

  printf("%d %d\n", input->shape[0][1], input->shape[0][2]);
  shape_net(pvanet);
  forward_net(pvanet);

  pvanet_get_output(boxes);
}
...
int main(int argc, char* argv[])
{
  ...
  Net frcnn;
  init_net(&frcnn);
  strcpy(frcnn.param_path, argv[1]);
  ...
  // load a text file containing image filenames to be tested
  {
    ...
    while (fgets(&buf[buf_count], 1024, fp_list)) {
      ...
      if (count == input->num_items) {
        ...
        get_output_frcnn_7_1_1(&frcnn, total_count);
